/**
 * Update podcast episodes with description tags
 *
 * Two modes:
 * 1. Normal flow: reads .show-notes-mapping.json from generate-show-notes step,
 *    updates episodes with pre-generated summaries.
 * 2. Backfill fallback: when no show notes mapping exists, queries relays for
 *    1 episode with a transcript but no description, downloads the transcript,
 *    generates show notes, and updates the episode.
 *
 * Every pipeline run processes at most 1 episode via backfill.
 */
...
function getPubkey(privateKey: string): string {
  if (privateKey.startsWith('nsec1')) {
    const decoded = nip19.decode(privateKey);
...
async function processShowNotesMapping(nostrPrivateKey: string): Promise<boolean> {
  ...
  try {
    ...
    return false; // No mapping file — fall through to backfill
  }

  const successfulNotes = showNotesResults.filter(r => r.success);
  if (successfulNotes.length === 0) {
    console.log('⏭️  Show notes mapping empty, no episodes to process');
    return false;
  }

  console.log(`📋 Found ${successfulNotes.length} show notes result(s) from pipeline`);
  ...
  console.log(`\n📊 Show notes processing: ${successCount} succeeded, ${failureCount} failed`);
  return true; // Handled — don't fall through to backfill
}

// --- Mode 2: Backfill 1 episode needing description ---

async function backfillOneEpisode(nostrPrivateKey: string): Promise<void> {
  console.log('\n🔄 No new show notes — checking for episodes needing descriptions (backfill mode)...');

  if (!process.env.ZHIPU_API_KEY) {
    console.log('⏭️  No ZHIPU_API_KEY — skipping backfill');
    return;
  }

  const pubkey = getPubkey(nostrPrivateKey);
  console.log(`🔑 Episode author pubkey: ${pubkey.substring(0, 8)}...`);

  // Fetch all kind 30054 episodes
  console.log('📡 Fetching episodes from relays...');
  ...
  // Filter: has transcript, no description — pick most recent
  ...
  console.log(`📝 ${needsDescription.length} episode(s) need descriptions, processing 1`);

  const event = needsDescription[0];
  ...
  const published = await publishEvent(updatedEvent);
  if (published) {
    console.log(`   ✅ Episode backfilled successfully`);
  } else {
    console.error(`   ❌ Failed to publish to any relay`);
  }
}
...
async function main() {
  ...
  if (dryRun) {
    console.log('🏃 DRY RUN: Would process episode descriptions');
    process.exit(0);
  }

  // Mode 1: Process pre-generated show notes from pipeline
  const handled = await processShowNotesMapping(nostrPrivateKey!);

  // Mode 2: Backfill 1 episode needing description
  if (!handled) {
    await backfillOneEpisode(nostrPrivateKey!);
  }
